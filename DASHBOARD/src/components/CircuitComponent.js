...
import { writeOpcUaData } from '../services/api';
...
const CircuitComponent = ({
    ...
    const [isEditingChamberSetpoint, setIsEditingChamberSetpoint] = useState(false);
    const [isEditingDischargerCurrent, setIsEditingDischargerCurrent] = useState(false);
    const [chamberSetpointValue, setChamberSetpointValue] = useState(ChamberSetpoint);
    const [dischargerCurrentValue, setDischargerCurrentValue] = useState(DischargerCurrent);

    useEffect(() => {
        setChamberSetpointValue(ChamberSetpoint);
        setDischargerCurrentValue(DischargerCurrent);
    }, [ChamberSetpoint, DischargerCurrent]);

    const handleChamberSetpointChange = (e) => {
        setChamberSetpointValue(e.target.value);
    };

    const handleDischargerCurrentChange = (e) => {
        setDischargerCurrentValue(e.target.value);
    };

    const handleChamberSetpointBlur = async () => {
        setIsEditingChamberSetpoint(false);
        await writeOpcUaData('ns=1;s=chamber.setpoint', chamberSetpointValue);
    };

    const handleDischargerCurrentBlur = async () => {
        setIsEditingDischargerCurrent(false);
        await writeOpcUaData('ns=1;s=battery.setpoint', dischargerCurrentValue);
    };
    ...
    return (
        <div style={{ position: 'relative', width: '1000px', height: '600px' }}>
            ...
            <input
                type="text"
                value={isEditingChamberSetpoint ? chamberSetpointValue : ChamberSetpoint}
                onChange={handleChamberSetpointChange}
                onBlur={handleChamberSetpointBlur}
                onFocus={() => setIsEditingChamberSetpoint(true)}
                style={{
                    position: 'absolute',
                    top: '335px',
                    left: '395px',
            ...
            <input
                type="text"
                value={isEditingDischargerCurrent ? dischargerCurrentValue : DischargerCurrent}
                onChange={handleDischargerCurrentChange}
                onBlur={handleDischargerCurrentBlur}
                onFocus={() => setIsEditingDischargerCurrent(true)}
                style={{
                    position: 'absolute',
                    top: '490px',
                    left: '840px',
